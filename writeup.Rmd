...
```{r, echo=FALSE, fig.cap="*Figure 2*: Difference in scheduled departure time vs. actual departure time as a percentage of maximum delay stratified by route."}
...
max.crossing.times <- 
  mean.times %>%
  group_by(route_name) %>%
  do(data.frame(max=max(.$crossing)))

mean.times <- merge(mean.times, max.crossing.times)
mean.times$percentage <- mean.times$crossing / mean.times$max
