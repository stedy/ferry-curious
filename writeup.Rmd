...
##On time departures
WSDOT uses GPS trackers on each ferry and is therefore able to record arrival and departure times to the second, so initially we looked at the difference in scheduled departure time versus actual departure time.
...
Each point represents the mean departure delay for that date for all ferries with a blue trendline overlaid. As a whole, the average daily delay in ferries was approximately 3 minutes for all ferry routes of interest in 2014. Overall variation in departure time is kept pretty minimal with a large bump in summer traffic.
...
Then we focused only on time delays over the course of the day
