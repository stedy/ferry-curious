...
First, a few caveats: Because WSDOT runs a [holiday schedule](http://www.wsdot.wa.gov/Ferries/infodesk/faq/general_info/) on most Federal holidays, we specifically accounted for those dates. Also, for this report we solely focused on routes in Central Puget Sound (ie non-Orcas Islands routes).
...
ggplot(all.days, aes(scheduled_departure, difference)) +
  geom_point(size=0.5) + stat_smooth() + xlab("Scheduled departure date") +
  ylab("Difference in scheduled vs. actual departure time \n in minutes")
```

In Figure 1, we used linear regression to add a trend line to graphically show any trends present in departure time delay. As a whole, the data shows minimal variation in departure time with a modest bump in departure delay towards the end of summer. In 2014, the average daily depature delay was approximately 3 minutes for all ferry routes of interest. There were some days where the ferry left early enough ahead of schedule to push the overall mean for that route on that day below zero although it is unclear why this happened.
...
all.days.max <- 
  all.days %>%
  group_by(route_name) %>%
  do(data.frame(max=max(.$difference)))

all.days <- merge(all.days, all.days.max)
all.days$percentage <- all.days$difference / all.days$max

ggplot(all.days, aes(scheduled_departure, percentage)) + geom_point(size=0.3) + stat_smooth() +
  scale_x_date(labels = scales::date_format("%b")) +
  facet_wrap(~route_name, scales="free_x") + xlab("Scheduled Departure Date") +
  ylab("Difference in scheduled vs. actual departure time \n as a percentage of maximum delay")
```

For Figure 2 we took the maximum delay for each route in 2014 and compared the daily average delay to that maximum value. This allows for a better insight into the daily delay values as well as normalizes all the routes for an easier comparison.
...
Same plot as above but just focusing on Seattle - Bainbridge Route

##Average crossing times
```{r, echo=FALSE, fig.cap="*Figure 5*: Average crossing time by route"}
raw <- 
  raw %>%
...
Figure 5 used the mean crossing time for each route for 2014 compared to the mean crossing time for that day to display changes in crossing times over the course of the year. Because this figure only looks at crossing times, any variation in trip length is due to factors that occurred while sailing.
